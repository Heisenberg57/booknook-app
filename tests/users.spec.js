...
test.describe('Users UI flow', () => {
  ...
  test('should add, view, edit and delete a user', async ({ page }) => {
    ...
    // ---- CREATE ----
    await page.fill('input[name="name"]', 'Test User 7');
    await page.fill('input[name="email"]', 'testuser7@example.com');
    await page.click('button:has-text("Add User")');

    // Verify user appears in list
    const newUserItem = page.getByText('Test User 7 - testuser7@example.com');
    ...
    await page.fill('input[name="name"]', 'Updated User 3');
    await page.fill('input[name="email"]', 'updated3@example.com');
    await page.click('button:has-text("Add User")');

    await expect(page.getByText('Updated User3 - updated3@example.com')).toBeVisible();
